...
class OpenRouterAgent(BaseAgent):
    """AI Agent powered by OpenRouter."""
    ...
    def _resolve_api_key(self) -> Optional[str]:
        ...
        try:
            ...
            stored = get_store().get_api_key("openrouter")
            if stored:
                return stored
        except ImportError:
            pass
    ...
    async def chat_stream(
        ...
        if not api_key:
            yield (
                "⚠️ **OpenRouter chưa được cấu hình.**\n\n"
                "Hãy vào **⚙️ Cài đặt → OpenRouter** và nhập API Key."
            )
            return

        try:
            from auth import get_store
            base_url = get_store().get_base_url("openrouter") or "https://openrouter.ai/api/v1"
        except Exception:
        ...
        # Referer headers required by OpenRouter
        extra_headers = {
            "HTTP-Referer": "http://localhost:8000",
            "X-Title": "AI Agent Chat Room",
        }
